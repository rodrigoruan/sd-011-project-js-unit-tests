...
const createMenu = (myMenu) => {
  restaurant = {
    ...
    pay: () => { 
      ...
      const consumptionOrder = restaurant.consumption;
        for (let index of consumptionOrder) {
          if (menu.drinks[index]) {
            total += menu.drinks[index];
        } else {
            total += menu.food[index];
        }
        }
       return parseFloat(total * 1.1).toPrecision(3);
    },
  };
  return restaurant;
};
