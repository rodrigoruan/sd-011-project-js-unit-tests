...
const bill = (menu) => {
  ...
  const { food, drink } = menu;
  let totalBill = 0;
  for (let i of consumption) {
    let itemPrice = menu.drink[i] ? menu.drink[i] : menu.food[i];
    totalBill += itemPrice;
  }
  return Math.round(totalBill * 1.1);
};
