...
function checkDrink(val, drink) {
  for (let index = 0; index < drink.length; index += 1) {
    if (val === drink[0][index]) {
...
function payMenu(menu) {
  ...
  for (let val of menu.consumption) {
    count += checkFood(val, food);
    count += checkDrink(val, drink);
  }
