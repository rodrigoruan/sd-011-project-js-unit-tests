...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    assert(productDetails('Alcool gel', 'Máscara').every(product => typeof product === 'object'));
    // Teste que os dois objetos são diferentes entre si.
    assert.notDeepStrictEqual(productDetails('Alcool gel', 'Máscara')[0], productDetails('Alcool gel', 'Mascara')[1]);
    // (Difícil) Teste que os dois productIds terminam com 123.
    assert(productDetails('Acool gel', 'Máscara').every(product => product.details.productId.endsWith(123)));
  });
});
