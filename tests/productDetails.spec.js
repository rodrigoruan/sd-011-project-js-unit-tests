...
describe('6 - Implemente os casos de teste para a função `productDetails`', () => {
  it('Verifica se a função `productDetails` tem o comportamento esperado', () => {
    ...
    assert(productDetails('Alcool Gel', 'Mascara').every(product => typeof product === 'object'));
    // Teste que os dois objetos são diferentes entre si.
    assert.notDeepStrictEqual(productDetails('Alcool Gel', 'Mascara')[0], productDetails('Alcool Gel', 'Mascara')[1]);
    // (Difícil) Teste que os dois productIds terminam com 123.
    assert(productDetails('Alcool Gel', 'Mascara').every(product => product.details.productId.slice(-3) === '123'));
  });
});
