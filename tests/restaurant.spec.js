...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    const meuRestaurante = createMenu({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    assert.deepStrictEqual(typeof meuRestaurante.fetchMenu(), 'object');
    ...
    const fetch = objetoRetornado.fetchMenu();
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    assert.deepStrictEqual(fetch, menu);
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer
    assert.deepStrictEqual(createMenu(objetoQualquer), objetoRetornado.fetchMenu());
