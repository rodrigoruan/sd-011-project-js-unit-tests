...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    let returnObj = createMenu({ food: {}, drink: {} });
    // const objetoRetornado = createMenu({ food: {}, drink: {} });
    // objetoRetornado.fetchMenu() // Retorno: { food: {}, drink: {}}
    // ```
    assert.deepStrictEqual(Object.keys(returnObj.fetchMenu()), ['food', 'drink']);
    // TESTE 3: Verifique que o menu passado pra função createMenu é identico ao menu recuperado pela função 'objetoRetornado.fetchMenu'
    // ```
    // const objetoRetornado = createMenu(objetoQualquer);
    // objetoRetornado.fetchMenu() // Retorno: objetoQualquer
    // ```
    let returnMenu = { food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} }
    assert.deepStrictEqual(createMenu(returnMenu).fetchMenu(), returnMenu);
    ...
    let restaurante = createMenu(returnMenu);
    ...
    restaurante = createMenu(returnMenu);
    restaurante.order('coxinha');
    restaurante.order('agua');
    restaurante.order('coxinha');
    ...
    restaurante.order('agua');
    restaurante.order('coxinha');
    assert.strictEqual(restaurante.pay(), 11.70)
