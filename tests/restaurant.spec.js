...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    assert.strictEqual(typeof (objetoRetornado.fetchMenu), 'function', 'erroooooou');
    ...
    const objetoVazio = {};
    assert.strictEqual(createMenu(objetoVazio).fetchMenu(), objetoVazio);
    // Agora faça o PASSO 1 no arquivo `src/restaurant.js`.
    // --------------------------------------------------------------------------------------
    // TESTE 4: Verifique que 'objetoRetornado.consumption', após a criação do menu, retorna um array vazio.
    const objetoRetornado3 = createMenu(objetoVazio);
    ...
    const objetoRetornado4 = createMenu(objetoVazio);
    ...
    const objetoRetornado5 = createMenu(objetoVazio);
    objetoRetornado5.order("coxinha");
    objetoRetornado5.order("agua");
    objetoRetornado5.order("sopa");
    objetoRetornado5.order("sashimi");
    assert.deepStrictEqual(objetoRetornado5.consumption, ["coxinha", "agua", "sopa", "sashimi"]);
    // Agora faça o TESTE 7 deste arquivo.
    // --------------------------------------------------------------------------------------
    // TESTE 7: Verifique que a função `order` aceita que pedidos repetidos sejam acrescidos a consumption.
    const objetoRetornado6 = createMenu(objetoVazio);
    objetoRetornado6.order('coxinha');
    objetoRetornado6.order('agua');
    objetoRetornado6.order('coxinha');
    assert.deepStrictEqual(objetoRetornado6.consumption, ['coxinha', 'agua', 'coxinha']);
