...
describe('9 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
    assert.strictEqual(typeof(createMenu()), typeof({ fetchMenu: () => {},}));
    ...
    // ```
    const object = createMenu(objetoQualquer).fetchMenu();
    ...
    // ```
    const testOrder = createMenu(objetoQualquer);
    testOrder.order("coxinha");
    testOrder.order("agua");
    testOrder.order("sopa");
    testOrder.order("sashimi");
    assert.deepStrictEqual(testOrder.consumption, ['coxinha', 'agua', 'sopa', 'sashimi']);
    ...
    // ```
    const testRepeat = createMenu(objetoRetornado);
    testRepeat.order('coxinha');
    testRepeat.order('agua');
    testRepeat.order('coxinha');
    assert.deepStrictEqual(testRepeat.consumption, ['coxinha', 'agua', 'coxinha']);
    ...
    // ```
    const newMenu = { food: { coxinha: 3.90, sanduiche: 9.90}, drink: {agua: 3.90, cerveja: 6.90} };
    const testPay = createMenu(newMenu);
    testPay.order('coxinha');
    testPay.order('agua');
    testPay.order('coxinha');
    assert.strictEqual(testPay.pay(), '12.87');
